...
{
    public class ExtractConversion
    {
        ...
        {
            return persons
                .GroupBy(eps => eps.Extract)
                .Select(g =>
                    new ExtractConversion
                    {
                        Extract = g.Key,
                        Persons = g
        ...
        public static void FillExtractItems(ExtractConversion[] extractConversions, ExtractDataContext dataContext)
        {
            foreach (var extractConversion in extractConversions)
            {
                extractConversion.FillExtractItems(dataContext);
            }
        }
        ...
        public static string[] AllPNRs(params ExtractConversion[] conversionExtracts)
        {
            var allExtractItems = conversionExtracts
                .SelectMany(pg => pg.Persons)
                .SelectMany(p => p.ExtractItems)
                .ToArray();

            return allExtractItems
                .Select(ei => ei.PNR)
                .Concat(allExtractItems.Select(ei => ei.RelationPNR))
                .Concat(allExtractItems.Select(ei => ei.RelationPNR2))
                .Distinct()
