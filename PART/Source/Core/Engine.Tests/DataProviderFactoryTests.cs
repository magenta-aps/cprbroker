...
{
    namespace DataProviderFactoryTests
    {
        public class Base
        {
            ...
            public DataProvider[] CreateDatabaseDataProviders(LocalProxyUsageOptions localProxyUsageOptions = LocalProxyUsageOptions.Default)
            {
                CustomProxyDataProvider._LocalProxyUsage = localProxyUsageOptions;
                return new Type[] { typeof(LocalDataProviderStub), typeof(CustomExternalDataProviderStub), typeof(CustomProxyDataProvider) }
        ...
        class CustomProxyDataProvider : CustomExternalDataProviderStub, ILocalProxyDataProvider
        {
            public static LocalProxyUsageOptions _LocalProxyUsage { get; set; }
            public LocalProxyUsageOptions LocalProxyUsage
            {
                get { return _LocalProxyUsage; }
                set { _LocalProxyUsage = value; }
            }
        }
        ...
        public class GetDataProviderList : Base
        {
            ...
            public void GetDataProviderList_LocalThenExternal_ReturnsBoth()
            {
                ...
                Assert.AreEqual(2, result.Where(p => p is IExternalDataProvider).Count());
                Assert.AreEqual(1, result.Where(p => !(p is IExternalDataProvider)).Count());
            }

            [Test]
            public void GetDataProviderList_PreLocal_ProxyFirst()
            {
                var result = new DataProviderFactory().GetDataProviderList(CreateConfigSection(), CreateDatabaseDataProviders( LocalProxyUsageOptions.BeforeLocal), typeof(IDataProviderStub), Schemas.SourceUsageOrder.LocalThenExternal);
                Assert.IsInstanceOf<CustomProxyDataProvider>(result.First());
            }

            [Test]
            public void GetDataProviderList_AfterLocal_ProxySecond()
            {
                var result = new DataProviderFactory().GetDataProviderList(CreateConfigSection(), CreateDatabaseDataProviders(LocalProxyUsageOptions.AfterLocal), typeof(IDataProviderStub), Schemas.SourceUsageOrder.LocalThenExternal);
                Assert.IsInstanceOf<LocalDataProviderStub>(result.First());
                Assert.IsInstanceOf<CustomProxyDataProvider>(result.Skip(1).First());
            }

            [Test]
            public void GetDataProviderList_Default_ProxyLast()
            {
                var result = new DataProviderFactory().GetDataProviderList(CreateConfigSection(), CreateDatabaseDataProviders(LocalProxyUsageOptions.Default), typeof(IDataProviderStub), Schemas.SourceUsageOrder.LocalThenExternal);
                Assert.IsInstanceOf<LocalDataProviderStub>(result.First());
                Assert.IsInstanceOf<CustomProxyDataProvider>(result.Skip(2).First());
            }
