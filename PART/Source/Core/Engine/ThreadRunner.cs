    ...
    public class ThreadRunner
    {
        public static void RunThreads(ThreadStart[] threadStarts, TimeSpan timeout)
        {
            ...
            else
            {
                ...
                var threads = threadStarts.Select(start => new Thread(
                    () =>
                    {
                ...
                Array.ForEach<Thread>(threads, th => th.Start());

                // Wait for sub results to continue
                DateTime executionStartTime = DateTime.Now;

                while (System.Threading.Interlocked.Read(ref finishedThreads) < threadStarts.Length && (DateTime.Now - executionStartTime) < timeout)
                {
